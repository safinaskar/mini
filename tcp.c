// IPv4, TCP, блокирующиеся сокеты, C99/C++
...
int mini_listen(const char *port){
	...
	int result = socket(AF_INET, SOCK_STREAM, 0); // <sys/socket.h> (в том числе AF_INET и SOCK_STREAM; there is no PF_INET in POSIX.1-2008)
...
int mini_connect(const char *host, const char *port){
	...
	int ai_error = getaddrinfo(host, port, &hints, &ai); // <netdb.h>
	...
	for(struct addrinfo *curr = ai; curr != 0; curr = curr->ai_next){
	...
	tcp_warn("%s:%s: all addresses returned by getaddrinfo failed, last connect(...) failed with the following message", host, port); /* После "the following message" будет добавлен strerror */
